...
# 读写编码字段文件
try:
    with open(dic_path, encoding="utf-8") as file:
        dic = json.load(file)
        print("字典载入完成")
except:
    print("字典载入失败")
...
# 多字段异常率衍生特征映射
def GroupByFeature_transform(column, data, newcolumn, dic):

    data[newcolumn] = data[column].map(dic.get(column))
    data[newcolumn] = data[newcolumn].fillna(-1)
...
for i in cat_columns:
    data_test = GroupByFeature_transform(column=i, data=data_test, newcolumn="ratio_Y_"+i, dic=dic)
...
data_test = data_test.drop(columns=cat_columns)

Y = data_test["Y"]
X = data_test.drop(columns=["Y"])
...
# 查看效果
PotRoc(Y=Y, preds=preds)
recall_precision_f1(Y=Y, preds=preds, k=10)
